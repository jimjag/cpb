/** @file test_full.c
 *
 * Tests the cpb encoder and decoder to be compatible with Google's
...
/** Checks an cpb return code. */
#define CHECK_CPB(err)                                                     \
    do {                                                                    \
        if (verbose)                                                        \
            printf("checking cpb result\n");                     \
        if ((err) != CPB_ERR_OK) {                                         \
            printf("%s [%d]: function returned: %d (%s)\n",       \
                   __FILE__, __LINE__, err, cpb_err_text(err));            \
            abort();                                                   \
        }                                                                   \
    } while (0)
                        ...
                        const char *filename, unsigned lineno)
{
    ...
    if (actual_len == expected_len)
        if (memcmp(actual_bytes, expected_bytes, actual_len) == 0)
            return;
    
    printf("%s [%d]: bytes value not as expected\n", filename, lineno);
    printf("actual (length = %zu):\n", actual_len);
    while (actual_len--)
        printf("0x%02x ", *actual_bytes++);
    printf("\n");
    printf("expected (length = %zu):\n", expected_len);
    while (expected_len--)
        printf("0x%02x ", *expected_bytes++);
    abort();
}
...
struct testing_fields {
    union {
        struct { cpb_enum_t test; } TestMessRequiredEnumSmall;
        struct { cpb_enum_t test; } TestMessRequiredEnum;
        ...
        struct { cpb_bool_t test; } TestMessRequiredBool;
        ...
        struct {
            ...
            float test_float;
            double test_double;
            cpb_bool_t test_boolean;
            cpb_enum_t test_enum_small;
            cpb_enum_t test_enum;
            char *test_string;
            u8_t *test_bytes;
...
static void generic_field_handler(struct cpb_decoder *decoder,
...
#define DO_TEST(value)                                                      \
    do {                                                                    \
        ...
        cpb_encoder_start(&encoder, foo_TestMessRequiredEnumSmall, buf, sizeof(buf)); \
        ret = cpb_encoder_add_enum(&encoder, foo_TestMessRequiredEnumSmall_test, FOO_##value); \
        CHECK_CPB(ret);                                                    \
        ...
        cpb_decoder_arg(&decoder, &fields);                                \
        cpb_decoder_field_handler(&decoder, generic_field_handler);        \
        ret = cpb_decoder_decode(&decoder, foo_TestMessRequiredEnumSmall, buf, len, &used); \
...
#define DO_TEST(value)                                                      \
    do {                                                                    \
        ...
        size_t len, used;                                                   \
        cpb_encoder_init(&encoder);                                        \
        cpb_encoder_start(&encoder, foo_TestMessRequiredEnum, buf, sizeof(buf)); \
        ret = cpb_encoder_add_enum(&encoder, foo_TestMessRequiredEnum_test, FOO_##value); \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
...
#define DO_TEST(field)                                                      \
    do {                                                                    \
        ...
        size_t len, used;                                                   \
        cpb_encoder_init(&encoder);                                        \
        cpb_encoder_start(&encoder, foo_TestFieldNo##field, buf, sizeof(buf)); \
        ret = cpb_encoder_add_string(&encoder, foo_TestFieldNo##field##_test, "tst"); \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
        ...
        cpb_decoder_arg(&decoder, &fields);                                \
        cpb_decoder_field_handler(&decoder, generic_field_handler);        \
        ret = cpb_decoder_decode(&decoder, foo_TestFieldNo##field, buf, len, &used); \
...
#define DO_TEST_REQUIRED(msg_type, cpb_type, value, vector)                \
    do {                                                                    \
        cpb_err_t ret;                                                     \
        struct cpb_encoder encoder;                                        \
        struct cpb_decoder decoder;                                        \
        ...
        size_t len, used;                                                   \
        cpb_encoder_init(&encoder);                                        \
        cpb_encoder_start(&encoder, foo_TestMessRequired##msg_type, buf, sizeof(buf)); \
        ret = cpb_encoder_add_##cpb_type(&encoder, foo_TestMessRequired##msg_type##_test, value); \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
        ...
        cpb_decoder_arg(&decoder, &fields);                                \
        cpb_decoder_field_handler(&decoder, generic_field_handler);        \
        ret = cpb_decoder_decode(&decoder, foo_TestMessRequired##msg_type, buf, len, &used); \
        CHECK_ASSERT(used == sizeof(vector), "not decoded all bytes");      \
        CHECK_CPB(ret);                                                    \
    } while (0);

#define DO_TEST_REQUIRED_BYTES(msg_type, cpb_type, value, value_len, vector) \
    do {                                                                    \
        cpb_err_t ret;                                                     \
        struct cpb_encoder encoder;                                        \
        struct cpb_decoder decoder;                                        \
        ...
        cpb_encoder_init(&encoder);                                        \
        cpb_encoder_start(&encoder, foo_TestMessRequired##msg_type, buf, sizeof(buf)); \
        ret = cpb_encoder_add_##cpb_type(&encoder, foo_TestMessRequired##msg_type##_test, (u8_t *) value, value_len); \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
        CHECK_BUF(buf, len, vector);                                        \
...
static void test_empty_optional(void)
{
    ...
    size_t len, used;
    cpb_encoder_init(&encoder);
    cpb_encoder_start(&encoder, foo_TestMessOptional, buf, sizeof(buf));
    len = cpb_encoder_finish(&encoder);
    CHECK_ASSERT(len == 0, "length must be null");
    cpb_decoder_init(&decoder);
    ret = cpb_decoder_decode(&decoder, foo_TestMessOptional, buf, len, &used);
    CHECK_ASSERT(used == 0, "not decoded all bytes");
    CHECK_CPB(ret);
}

#define DO_TEST_OPTIONAL(cpb_type, field, value, vector)                   \
    do {                                                                    \
        cpb_err_t ret;                                                     \
        struct cpb_encoder encoder;                                        \
        struct cpb_decoder decoder;                                        \
        ...
        cpb_encoder_init(&encoder);                                        \
        cpb_encoder_start(&encoder, foo_TestMessOptional, buf, sizeof(buf)); \
        ret = cpb_encoder_add_##cpb_type(&encoder, foo_TestMessOptional_test_##field, value); \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
        CHECK_BUF(buf, len, vector);                                        \
...
#define DO_TEST_OPTIONAL_BYTES(cpb_type, field, value, value_len, vector)  \
    do {                                                                    \
        cpb_err_t ret;                                                     \
        struct cpb_encoder encoder;                                        \
        struct cpb_decoder decoder;                                        \
        ...
        cpb_encoder_init(&encoder);                                        \
        cpb_encoder_start(&encoder, foo_TestMessOptional, buf, sizeof(buf)); \
        ret = cpb_encoder_add_##cpb_type(&encoder, foo_TestMessOptional_test_##field, (u8_t *) value, value_len); \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
        CHECK_BUF(buf, len, vector);                                        \
...
static void test_empty_repeated(void)
{
    ...
    size_t len, used;
    cpb_encoder_init(&encoder);
    cpb_encoder_start(&encoder, foo_TestMess, buf, sizeof(buf));
    len = cpb_encoder_finish(&encoder);
    CHECK_ASSERT(len == 0, "length must be null");
    cpb_decoder_init(&decoder);
    ret = cpb_decoder_decode(&decoder, foo_TestMess, buf, len, &used);
    CHECK_ASSERT(used == 0, "not decoded all bytes");
    CHECK_CPB(ret);
}

#define DO_TEST_REPEATED(cpb_type, field, array, vector)                   \
    do {                                                                    \
        cpb_err_t ret;                                                     \
        struct cpb_encoder encoder;                                        \
        struct cpb_decoder decoder;                                        \
        ...
        for (i = 0; i < ARRAY_SIZE(array); i++)                             \
            ret = cpb_encoder_add_##cpb_type(&encoder, foo_TestMess_test_##field, array[i]); \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
        CHECK_BUF(buf, len, vector);                                        \
...
static void test_empty_packed_repeated(void)
{
    ...
    size_t len, used;
    cpb_encoder_init(&encoder);
    cpb_encoder_start(&encoder, foo_TestMessPacked, buf, sizeof(buf));
    len = cpb_encoder_finish(&encoder);
    CHECK_ASSERT(len == 0, "length must be null");
    cpb_decoder_init(&decoder);
    ret = cpb_decoder_decode(&decoder, foo_TestMessPacked, buf, len, &used);
    CHECK_ASSERT(used == 0, "not decoded all bytes");
    CHECK_CPB(ret);
}

#define DO_TEST_PACKED_REPEATED(cpb_type, field, array, vector)            \
    do {                                                                    \
        cpb_err_t ret;                                                     \
        struct cpb_encoder encoder;                                        \
        struct cpb_decoder decoder;                                        \
        ...
        cpb_encoder_init(&encoder);                                        \
        cpb_encoder_start(&encoder, foo_TestMessPacked, buf, sizeof(buf)); \
        ret = cpb_encoder_packed_repeated_start(&encoder, foo_TestMessPacked_test_##field); \
        CHECK_CPB(ret);                                                    \
        for (i = 0; i < ARRAY_SIZE(array); i++)                             \
            ret = cpb_encoder_add_##cpb_type(&encoder, foo_TestMessPacked_test_##field, array[i]); \
        CHECK_CPB(ret);                                                    \
        ret = cpb_encoder_packed_repeated_end(&encoder);                   \
        CHECK_CPB(ret);                                                    \
        len = cpb_encoder_finish(&encoder);                                \
        CHECK_BUF(buf, len, vector);                                        \
